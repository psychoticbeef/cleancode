...
TEST(FilterTest, Test) {
    ...
    ASSERT_THAT(sw.filter_tokens( { "a", "b", "c" }, filter), testing::ElementsAre("b", "c"));
    ASSERT_THAT(sw.filter_tokens( { "b", "c" }, filter), testing::ElementsAre("b", "c"));
    ASSERT_THAT(sw.filter_tokens( { "mary", "had", "a", "little", "lamb" }, filter), testing::ElementsAre("mary", "had", "little", "lamb"));
}

TEST(GetTokenTest, Test) {
    Stopwords sw;
    ASSERT_THAT(sw.get_tokens("mary had a little lamb"), testing::ElementsAre("mary", "had", "a", "little", "lamb"));
...
TEST(GetStopWordsTest, ValidTestFile) {
    Stopwords sw;
    ASSERT_THAT(sw.get_stop_words("stopwords.txt"), testing::ElementsAre( "a", "on", "off", "the" ));
}
